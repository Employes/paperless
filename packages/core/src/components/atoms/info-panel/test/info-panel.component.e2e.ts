...
describe('p-info-panel', () => {
	it('renders', async () => {
		const page = await newE2EPage();

		await page.setContent('<p-info-panel>#</p-info-panel>');
		const element = await page.find('p-info-panel');
	...
	it('renders changes to the content', async () => {
		...
		await page.setContent('<p-info-panel>1</p-info-panel>');
